...
```text
あなたは世界中から絶賛されているプレゼンテーションの達人です。「これから広がる生成 AI の未来」というテーマの30分のプレゼンの構成案を2つ考えてください。それぞれの案に対して、どのような聴衆を想定しているかも教えてください。
```

> Tip  
> 参加者の属性があらかじめわかっていれば、その情報を追加したり、これは外せないよね、というトピックがあればそれを追加するなどすると、より良い構成案を出力させることができます。
...
```text
あなたは、スポーツショップの凄腕マーケティング担当者です。以下の特徴を持つお客様に対して、自社のセールに来てもらうためのメール文面を作成してください。

{ここにお客様の特徴テキストをコピー}
```

> Tip  
> LLM に役割を与えることで、それっぽい文面を作ってくれるようになります。
...
このケースの場合、dosukoi というラベルは一般的ではないので、Task 7 のようなシンプルなプロンプトではうまくいかないはずです。
...
> Tip  
> 出力形式の表現に手こずりましたが、抽出したテキストなどで置き換えて欲しい部分を [人名 or 場所 or 体の部位のいずれか] のように [ ] で囲むといい感じになりました。

</details>

### Task 12: ガイドラインに沿ってテキストを添削（要 Claude 3 Sonnet）
...
```text
あなたは文章校正のプロフェッショナルです。<text>タグの中に記載されたテキストが、<guideline>タグの中にマークダウン形式で記載されたガイドラインに沿っているかを確認し、ガイドラインに沿っていない部分を修正した文章を出力してください。ガイドラインに反していない部分は修正しないでください。必ず<format>タグの形式に沿って出力してください。
...
ここに校正後の文章を記載する。
## コメント
ここに校正によって修正された箇所とその理由を以下の形式で記載する。
